...
class SalesforceUserRetrieveV1
  ...
  def execute()
    ...
    uri = URI.parse("https://#{@instance}.salesforce.com/services/data/v27.0/sobjects/User/#{@parameters['account_id']}")
    ...
    <results>
      ...
      <result name="LastName">#{result['LastName']}</result>
      <result name="FirstName">#{result['FirstName']}</result>
      <result name="Name">#{result['Name']}</result>
      <result name="CommunityNickname">#{result['CommunityNickname']}</result>
