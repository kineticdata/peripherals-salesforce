...
class SalesforceOpportunityCreateV2
  ...
  def execute()
    ...
    puts "Attempting to create an Opportunity using the built Opportunity JSON Object: #{opportunity_object.to_json}" if @enable_debug_logging
    begin
      resp = RestClient.post("#{salesforce_instance}/services/data/v#{@api_version}/sobjects/Opportunity", opportunity_object.to_json,
        :content_type => "application/json", :accept => :json, :authorization => "OAuth #{access_token}")
    rescue RestClient::BadRequest => error
      puts "Error encountered while attempting to create an Opportunity:\n#{error.http_body}"
    ...
    puts "Opportunity '#{opportunity_id}' successfully created" if @enable_debug_logging

    # Return results which is an XML String
    <<-RESULTS
    <results>
      <result name="Opportunity Id">#{escape(opportunity_id)}</result>
    </results>
    RESULTS
  end
